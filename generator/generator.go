...
var SolTypeMap = map[string]string{
	...
	"uint256":         "bytes",
	//"uint256":            "uint64",
	// solidity uint256 is an alias to uint256. but we add our own schema to it.
	// and only use uint256 for amount in wei. use uint256 for uint64 which could in theory save some gas.
	// eg. proto type uint64, soltype uint256. Note without uint256 soltype it also works, only a bit more gas.
}
...
func (g *Generator) generateHeader(f fdes) {
	g.P("// SPDX-License-Identifier: GPL-3.0-only")
	g.P("")
...
func (g *Generator) generateMsg(m msgdes) {
	...
	// repeated uint256 doesn't need this because it's packed
...
func getSolDecodeStr(field *descriptor.FieldDescriptorProto, soltype string) (code string) {
	...
	if isPacked {
		// buf.decPacked return uint256[], use Pb.uintXXs to convert to uintXX[]
		if soltype == "uint256" {
...
library Pb {
    ...
    function decVarint(Buffer memory buf) internal pure returns (uint256 v) {
        ...
        v = buf.idx;  // use v to save one additional uint256 variable
        assembly {
